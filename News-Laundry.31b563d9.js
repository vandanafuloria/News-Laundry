...
function addToDom(news) {
    for(let i = 1; i < 4; i++){
        ...
        if (url.includes("biztoc.com") || url.includes("english.khabarhub.com")) continue;
...
function MoveTosearchContent() {
    ...
    getLatestNews();
    latestNewsContainerEl.scrollIntoView({
        behavior: "smooth"
    });
}
...
function getLatestNews(e) {
    ...
    return fetch(urlLatest).then((res)=>{
        return res.json();
    }).then((response)=>{
...
},{"./styles.css":"lW6qc"}],"lW6qc":[function() {},{}]},["5j6Kf","a0t4e"], "a0t4e", "parcelRequire9012", {})

//# sourceMappingURL=News-Laundry.31b563d9.js.map
